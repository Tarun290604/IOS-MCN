...
//------------------------------------------------------------------------------
{
  ngap_gNB_instance_t *instance_p = NULL;
  NGAP_NGAP_PDU_t pdu;
  NGAP_PathSwitchRequest_t *input;
  NGAP_PathSwitchRequestIEs_t *ie;
  struct ngap_gNB_amf_data_s *amf_desc_p = NULL;
  uint8_t *buffer = NULL;
  uint32_t length;
  struct ngap_gNB_ue_context_s *ue_context_p = NULL;
  NGAP_PDUSessionResourceToBeSwitchedDLItem_t *pdustobeswitched = NULL;

  instance_p = ngap_gNB_get_instance(instance);
  DevAssert(instance_p != NULL);

  amf_desc_p = ngap_gNB_nnsf_select_amf_by_guami_no_cause(instance_p, path_switch_req_p->ue_guami);
  if (amf_desc_p == NULL) {
    NGAP_WARN("No AMF is associated to the gNB\n");
    return -1;
  }
  ...
  /* Keep a reference to the selected AMF */
  ue_desc_p->amf_ref = amf_desc_p;
  ue_desc_p->gNB_ue_ngap_id = path_switch_req_p->gNB_ue_ngap_id;
  ue_desc_p->amf_ue_ngap_id = path_switch_req_p->amf_ue_ngap_id;
  ue_desc_p->gNB_instance = instance_p;
  ue_desc_p->ue_initial_id = path_switch_req_p->ue_initial_id;
  // insert in master table
  ngap_store_ue_context(ue_desc_p);

  if ((ue_context_p = ngap_get_ue_context(path_switch_req_p->gNB_ue_ngap_id)) == NULL) {
    // The context for this gNB ue ngap id doesn't exist in the map of gNB UEs
  ...
  input = &pdu.choice.initiatingMessage->value.choice.PathSwitchRequest;
  ...
  ie->value.choice.RAN_UE_NGAP_ID = path_switch_req_p->gNB_ue_ngap_id;
  asn1cSeqAdd(&input->protocolIEs.list, ie);
  ...
  asn_uint642INTEGER(&ie->value.choice.AMF_UE_NGAP_ID, path_switch_req_p->amf_ue_ngap_id);
  asn1cSeqAdd(&input->protocolIEs.list, ie);
  ...
  MACRO_GNB_ID_TO_CELL_IDENTITY(instance_p->gNB_id,
                                0, // Cell ID
                                &userinfo_nr_p->nR_CGI.nRCellIdentity);
  MCC_MNC_TO_PLMNID(path_switch_req_p->ue_guami.mcc,
                    path_switch_req_p->ue_guami.mnc,
                    path_switch_req_p->ue_guami.mnc_len,
                    &userinfo_nr_p->nR_CGI.pLMNIdentity);
  ...
  ENCRALG_TO_BIT_STRING(path_switch_req_p->security_capabilities.nRencryption_algorithms,
                        &ie->value.choice.UESecurityCapabilities.nRencryptionAlgorithms);
  ENCRALG_TO_BIT_STRING(path_switch_req_p->security_capabilities.eUTRAencryption_algorithms,
                        &ie->value.choice.UESecurityCapabilities.eUTRAencryptionAlgorithms);
  INTPROTALG_TO_BIT_STRING(path_switch_req_p->security_capabilities.nRintegrity_algorithms,
                           &ie->value.choice.UESecurityCapabilities.nRintegrityProtectionAlgorithms);
  INTPROTALG_TO_BIT_STRING(path_switch_req_p->security_capabilities.eUTRAintegrity_algorithms,
                           &ie->value.choice.UESecurityCapabilities.eUTRAintegrityProtectionAlgorithms);
  asn1cSeqAdd(&input->protocolIEs.list, ie);

  /* PDUSessionResourceToBeSwitchedDLList - mandatory */
  if (path_switch_req_p->nb_of_pdusessions > 0) {
    ...
    for (int i = 0; i < path_switch_req_p->nb_of_pdusessions; i++) {
      pdustobeswitched =
          (NGAP_PDUSessionResourceToBeSwitchedDLItem_t *)calloc(1, sizeof(NGAP_PDUSessionResourceToBeSwitchedDLItem_t));
      pdustobeswitched->pDUSessionID = path_switch_req_p->pdusessions_tobeswitched[i].pdusession_id;

      /* Path Switch Request Transfer */
      /* 1. DL NG-U UP TNL Information */
      NGAP_PathSwitchRequestTransfer_t pathswitchtransfer = {0};

      pathswitchtransfer.dL_NGU_UP_TNLInformation.present = NGAP_UPTransportLayerInformation_PR_gTPTunnel;
      asn1cCalloc(pathswitchtransfer.dL_NGU_UP_TNLInformation.choice.gTPTunnel, tmp);

      TRANSPORT_LAYER_ADDRESS_IPv4_TO_BIT_STRING((*(long *)path_switch_req_p->pdusessions_tobeswitched[i].gNB_addr.buffer),
                                                 &tmp->transportLayerAddress);

      INT32_TO_OCTET_STRING(path_switch_req_p->pdusessions_tobeswitched[i].gtp_teid, &tmp->gTP_TEID);

      printf("path_switch_req: pdusession ID %ld, gnb_addr %d.%d.%d.%d, SIZE %ld, TEID %u\n",
             pdustobeswitched->pDUSessionID,
             tmp->transportLayerAddress.buf[0],
             tmp->transportLayerAddress.buf[1],
             tmp->transportLayerAddress.buf[2],
             tmp->transportLayerAddress.buf[3],
      ...
      /* 2. QoS Flow Accepted List */
      for (int j = 0; j < path_switch_req_p->pdusessions_tobeswitched[i].nb_of_qos_flow; j++) {
        // NGAP_QosFlowAcceptedItem_t *accepted_qos = (NGAP_QosFlowAcceptedItem_t *)calloc(1, sizeof(NGAP_QosFlowAcceptedItem_t));
        // accepted_qos = path_switch_req_p->pdusessions_tobeswitched[i].associated_qos_flows;
        // asn1cCalloc(pathswitchtransfer.qosFlowAcceptedList.list, accepted_qos);
        // accepted_qos = (NGAP_QosFlowAcceptedItem_t *)calloc(1, sizeof(NGAP_QosFlowAcceptedItem_t));
        asn1cSequenceAdd(pathswitchtransfer.qosFlowAcceptedList.list, NGAP_QosFlowAcceptedItem_t, accepted_qos);
        accepted_qos->qosFlowIdentifier = (long)(path_switch_req_p->pdusessions_tobeswitched[i].associated_qos_flows[j].qfi);
      }
      asn_encode_to_new_buffer_result_t res =
          asn_encode_to_new_buffer(NULL, ATS_ALIGNED_CANONICAL_PER, &asn_DEF_NGAP_PathSwitchRequestTransfer, &pathswitchtransfer);
      AssertFatal(res.buffer, "ASN1 message encoding failed (%s, %lu)!\n", res.result.failed_type->name, res.result.encoded);
      pdustobeswitched->pathSwitchRequestTransfer.size = res.result.encoded;
      pdustobeswitched->pathSwitchRequestTransfer.buf = res.buffer;
      OCTET_STRING_fromBuf(&pdustobeswitched->pathSwitchRequestTransfer, res.buffer, res.result.encoded);
      // ASN_STRUCT_FREE_CONTENTS_ONLY(asn_DEF_NGAP_PathSwitchRequestTransfer, &pathswitchtransfer);
      asn1cSeqAdd(&ie->value.choice.PDUSessionResourceToBeSwitchedDLList.list, pdustobeswitched);
    }
    asn1cSeqAdd(&input->protocolIEs.list, ie);
  }
  ...
  if (ngap_gNB_encode_pdu(&pdu, &buffer, &length) < 0) {
    /* Failed to encode message */
    DevMessage("Failed to encode Path Switch Request\n");
    return -1;
  }
  LOG_I(NR_RRC,
        "ngap_gNB_itti_send_sctp_data_req!!!!!! assoc id %d, length %d, stream %d\n",
        ue_context_p->amf_ref->assoc_id,
        length,
        ue_context_p->tx_stream);
  ngap_gNB_itti_send_sctp_data_req(instance_p->instance, ue_context_p->amf_ref->assoc_id, buffer, length, ue_context_p->tx_stream);
