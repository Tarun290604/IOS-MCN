...
static int ngap_gNB_handle_ng_path_switch_request_ack(sctp_assoc_t assoc_id, uint32_t stream, NGAP_NGAP_PDU_t *pdu)
{
  uint64_t amf_ue_ngap_id;
  ngap_gNB_amf_data_t *amf_desc_p = NULL;
  NGAP_PathSwitchRequestAcknowledge_t *container;
  ...
  if ((amf_desc_p = ngap_gNB_get_AMF(NULL, assoc_id, 0)) == NULL) {
    NGAP_ERROR(
        "[SCTP %u] Received Path Switch Request Ack for non "
        "existing AMF context\n",
        assoc_id);
    return -1;
  }
  ...
  if (ie == NULL) {
    NGAP_ERROR("[SCTP %u] Received path switch request ack for ie is NULL\n", assoc_id);
    itti_free(ITTI_MSG_ORIGIN_ID(message_p), message_p);
    return -1;
  }
  msg->next_hop_chain_count = ie->value.choice.SecurityContext.nextHopChainingCount;
  ...
  NGAP_FIND_PROTOCOLIE_BY_ID(NGAP_PathSwitchRequestAcknowledgeIEs_t,
                             ...
                             true);
  if (ie != NULL) {
    msg->nb_pdusessions_switched = ie->value.choice.PDUSessionResourceSwitchedList.list.count;
    for (int i = 0; i < ie->value.choice.PDUSessionResourceSwitchedList.list.count; i++) {
      NGAP_PDUSessionResourceSwitchedItem_t *item_p = ie->value.choice.PDUSessionResourceSwitchedList.list.array[i];
      msg->pdusessions_switched[i].pdusession_id = item_p->pDUSessionID;
      allocCopy(&msg->pdusessions_switched[i].pdusessionTransfer, item_p->pathSwitchRequestAcknowledgeTransfer);
    }
  }
  ...
  NGAP_FIND_PROTOCOLIE_BY_ID(NGAP_PathSwitchRequestAcknowledgeIEs_t,
                             ie,
                             container,
                             NGAP_ProtocolIE_ID_id_PDUSessionResourceReleasedListPSAck,
                             false);
  if (ie) {
    msg->nb_pdusessions_tobereleased = ie->value.choice.PDUSessionResourceReleasedListPSAck.list.count;
    for (int i = 0; i < ie->value.choice.PDUSessionResourceReleasedListPSAck.list.count; i++) {
      NGAP_PDUSessionResourceReleasedItemPSAck_t *releasePDU = ie->value.choice.PDUSessionResourceReleasedListPSAck.list.array[i];
      msg->pdusessions_tobereleased[i].pdusession_id = releasePDU->pDUSessionID;
    }
  } else {
    NGAP_WARN("PDUSessionToBeReleasedList not supported\n");
    msg->nb_pdusessions_tobereleased = 0;
  }
  ...
  for (int i = 0; i < ie->value.choice.AllowedNSSAI.list.count; i++) {
    nssai_item = ie->value.choice.AllowedNSSAI.list.array[i];
    OCTET_STRING_TO_INT8(&nssai_item->s_NSSAI.sST, msg->allowed_nssai[i].sst);
    if (nssai_item->s_NSSAI.sD != NULL) {
      msg->allowed_nssai[i].sd = 0;
      BUFFER_TO_INT24((uint8_t *)nssai_item->s_NSSAI.sD->buf, msg->allowed_nssai[i].sd);
    } else {
      msg->allowed_nssai[i].sd = 0xffffff;
    }
  }
  ...
  LOG_I(NGAP,"Received Path Switch Request Ack\n");
  itti_send_msg_to_task(TASK_RRC_GNB, ue_desc_p->gNB_instance->instance, message_p);
  return 0;
}
