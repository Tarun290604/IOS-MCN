...
void ue_context_setup_request(const f1ap_ue_context_setup_t *req)
{
  ...
  if (!ue_id_provided) {
    ...
    UE = find_nr_UE(&mac->UE_info, req->gNB_DU_ue_id);
      if(UE == NULL) {
        resp.cause = F1AP_CAUSE_RADIO_NETWORK;
        resp.cause_value = F1AP_CauseRadioNetwork_unknown_or_already_allocated_gnb_du_ue_f1ap_id;
        mac->mac_rrc.ue_context_setup_failure(&resp);
        return;
      }
  }
