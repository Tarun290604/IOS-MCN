...
static void ue_context_setup_failure_f1ap(const f1ap_ue_context_setup_t* fail)
{
  MessageDef *msg = itti_alloc_new_message(TASK_MAC_GNB, 0, F1AP_UE_CONTEXT_SETUP_FAILURE);
  f1ap_ue_context_setup_t *f1ap_msg = &F1AP_UE_CONTEXT_SETUP_FAILURE(msg);
  *f1ap_msg = *fail;
  itti_send_msg_to_task(TASK_DU_F1, 0, msg);
}
...
void mac_rrc_ul_f1ap_init(struct nr_mac_rrc_ul_if_s *mac_rrc)
{
  ...
  mac_rrc->ue_context_setup_failure = ue_context_setup_failure_f1ap;
