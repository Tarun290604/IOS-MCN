...
{
  MessageDef                       *msg_p;
  F1AP_UEContextSetupFailure_t    *container;
  F1AP_UEContextSetupFailureIEs_t *ie;

  container = &pdu->choice.unsuccessfulOutcome->value.choice.UEContextSetupFailure;
  msg_p = itti_alloc_new_message(TASK_DU_F1, 0,  F1AP_UE_CONTEXT_SETUP_FAILURE);
  msg_p->ittiMsgHeader.originInstance = assoc_id;
  f1ap_ue_context_setup_t *fail = &F1AP_UE_CONTEXT_SETUP_FAILURE(msg_p);

  /* GNB_CU_UE_F1AP_ID */
  F1AP_FIND_PROTOCOLIE_BY_ID(F1AP_UEContextSetupFailureIEs_t, ie, container,
                             F1AP_ProtocolIE_ID_id_gNB_CU_UE_F1AP_ID, true);
  fail->gNB_CU_ue_id = ie->value.choice.GNB_CU_UE_F1AP_ID;
  LOG_D(F1AP, "fail->gNB_CU_ue_id is: %d \n", fail->gNB_CU_ue_id);

  /* GNB_DU_UE_F1AP_ID */
  F1AP_FIND_PROTOCOLIE_BY_ID(F1AP_UEContextSetupFailureIEs_t, ie, container,
                             F1AP_ProtocolIE_ID_id_gNB_DU_UE_F1AP_ID, true);
  fail->gNB_DU_ue_id = ie->value.choice.GNB_DU_UE_F1AP_ID;
  LOG_D(F1AP, "fail->gNB_DU_ue_id is: %d \n", fail->gNB_DU_ue_id);
  ...
  switch(ie->value.choice.Cause.present)
  {
    case F1AP_Cause_PR_radioNetwork:
      ...
      fail->cause_value = ie->value.choice.Cause.choice.radioNetwork;
      break;
    case F1AP_Cause_PR_transport:
      fail->cause = F1AP_CAUSE_TRANSPORT;
      fail->cause_value = ie->value.choice.Cause.choice.transport;
      break;
    case F1AP_Cause_PR_protocol:
      fail->cause = F1AP_CAUSE_PROTOCOL;
      fail->cause_value = ie->value.choice.Cause.choice.protocol;
      break;
    case F1AP_Cause_PR_misc:
      fail->cause = F1AP_CAUSE_MISC;
      fail->cause_value = ie->value.choice.Cause.choice.misc;
      break;
    case F1AP_Cause_PR_NOTHING:
    default:
      fail->cause = F1AP_CAUSE_NOTHING;
      break;
  }
