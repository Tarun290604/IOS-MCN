...
void trigger_bearer_setup(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, int n, pdusession_t *sessions, uint64_t ueAggMaxBitRateDownlink)
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (UE->ho_context_xn != NULL) {
      session->gtp_teid = sessions[i].gtp_teid;
      memcpy(session->upf_addr.buffer, sessions[i].upf_addr.buffer, sizeof(uint8_t) * 4);
    }
    if (cuup_nssai.sst == 0)
      cuup_nssai = pdu->nssai; /* for CU-UP selection below */

    cuup_nssai = pdu->nssai;
...
//------------------------------------------------------------------------------
void rrc_gNB_send_NGAP_PATH_SWITCH_REQ(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, uint8_t xid)
//------------------------------------------------------------------------------
{
  MessageDef *msg_p;
  int pdu_sessions_done = 0;

  msg_p = itti_alloc_new_message(TASK_RRC_GNB, 0, NGAP_PATH_SWITCH_REQ);
  ...
  for (int pdusession = 0; pdusession < UE->nb_of_pdusessions; pdusession++) {
    rrc_pdu_session_param_t *session = &UE->pduSession[pdusession];
    if (session->status == PDU_SESSION_STATUS_REESTABLISHED) {
      pdu_sessions_done++;
      req->pdusessions_tobeswitched[pdusession].pdusession_id = session->param.pdusession_id;
      req->pdusessions_tobeswitched[pdusession].pdu_session_type = session->param.pdu_session_type;
      req->pdusessions_tobeswitched[pdusession].nb_of_qos_flow = session->param.nb_qos;
      req->pdusessions_tobeswitched[pdusession].gtp_teid = session->param.gNB_teid_N3;
      memcpy(req->pdusessions_tobeswitched[pdusession].gNB_addr.buffer,
             session->param.gNB_addr_N3.buffer,
             sizeof(req->pdusessions_tobeswitched[pdusession].gNB_addr.buffer));
      req->pdusessions_tobeswitched[pdusession].gNB_addr.length = session->param.gNB_addr_N3.length;
      for (int qos_flow_index = 0; qos_flow_index < session->param.nb_qos; qos_flow_index++) {
        req->pdusessions_tobeswitched[pdusession].associated_qos_flows[qos_flow_index].qfi = session->param.qos[qos_flow_index].qfi;
      }
      LOG_I(NR_RRC,
            "msg index %d, pdu_sessions index %d, status %d): nb_of_pdusessions %d,  pdusession_id %d, teid: %u \n ",
            pdu_sessions_done,
            pdusession,
            session->status,
            UE->nb_of_pdusessions,
            req->pdusessions_tobeswitched[pdusession].pdusession_id,
            req->pdusessions_tobeswitched[pdusession].gtp_teid);
      req->nb_of_pdusessions = pdu_sessions_done;
    }
  }

  /* UE security capabilities */
  req->security_capabilities.nRencryption_algorithms = UE->security_capabilities.nRencryption_algorithms;
  req->security_capabilities.nRintegrity_algorithms = UE->security_capabilities.nRintegrity_algorithms;
  req->security_capabilities.eUTRAencryption_algorithms = UE->security_capabilities.eUTRAencryption_algorithms;
  req->security_capabilities.eUTRAintegrity_algorithms = UE->security_capabilities.eUTRAintegrity_algorithms;
  /* User Location Information*/
  req->ue_guami.mcc = UE->ue_guami.mcc;
  req->ue_guami.mnc = UE->ue_guami.mnc;
  req->ue_guami.mnc_len = UE->ue_guami.mnc_len;
  req->ue_guami.amf_region_id = UE->ue_guami.amf_region_id;
  req->ue_guami.amf_set_id = UE->ue_guami.amf_set_id;
  req->ue_guami.amf_pointer = UE->ue_guami.amf_pointer;

  LOG_I(NR_RRC, "NGAP_PATH_SWITCH_REQ: sending the message\n");
  itti_send_msg_to_task(TASK_NGAP, rrc->module_id, msg_p);
  return;
}

//-----------------------------------------------------------------------------------------------
void rrc_gNB_process_NGAP_PATH_SWITCH_REQ_ACK(MessageDef *msg_p, instance_t instance)
//----------------------------------------------------------------------------------------------
{
  protocol_ctxt_t ctxt = {0};
  ngap_path_switch_req_ack_t *ack = &NGAP_PATH_SWITCH_REQ_ACK(msg_p);
  rrc_gNB_ue_context_t *ue_context_p = rrc_gNB_get_ue_context(RC.nrrrc[instance], ack->gNB_ue_ngap_id);
  gNB_RRC_UE_t *UE = &ue_context_p->ue_context;
  ...
  /*UE ids */
  UE->rrc_ue_id = ack->gNB_ue_ngap_id;
  UE->amf_ue_ngap_id = ack->amf_ue_ngap_id;

  LOG_I(RRC, "[gNB %ld] Received NGAP_PATH_SWITCH_REQ_ACK gNB_ue_ngap_id %d\n", instance, ack->gNB_ue_ngap_id);

  if (ue_context_p == NULL) {
    LOG_W(RRC, "[gNB %ld] In NGAP_PATH_SWITCH_REQ_ACK: unknown UE from NGAP ids (%d)\n", instance, ack->gNB_ue_ngap_id);
    return;
  }
  ...
  UE->nh_ncc = ack->next_hop_chain_count;
  memcpy(UE->nh, &ack->next_security_key, 32);

  /* UE security capabilities */
  UE->security_capabilities.nRencryption_algorithms = ack->security_capabilities.nRencryption_algorithms;
  UE->security_capabilities.nRintegrity_algorithms = ack->security_capabilities.nRintegrity_algorithms;
  UE->security_capabilities.eUTRAencryption_algorithms = ack->security_capabilities.eUTRAencryption_algorithms;
  UE->security_capabilities.eUTRAintegrity_algorithms = ack->security_capabilities.eUTRAintegrity_algorithms;

  if (ack->nb_pdusessions_switched > 0) {
    UE->nb_of_pdusessions = ack->nb_pdusessions_switched;
    rrc_pdu_session_param_t *pduSession = calloc(UE->nb_of_pdusessions, sizeof(*UE->pduSession));
    // UE->pduSession = calloc(UE->nb_of_pdusessions, sizeof(UE->pduSession));
    AssertFatal(UE->pduSession != NULL, "out of memory\n");
    for (int i = 0; i < UE->nb_of_pdusessions; ++i) {
      UE->pduSession[i] = pduSession[i];
      UE->pduSession[i].param = ack->pdusessions_switched[i];
    }
  }
  if (ack->nb_pdusessions_tobereleased > 0) {
    for (int i = 0; i < ack->nb_pdusessions_tobereleased; ++i) {
      pdusession_failed_t *f = &ack->pdusessions_tobereleased[i];
      f->pdusession_id = ack->pdusessions_tobereleased[i].pdusession_id;
      f->cause = NGAP_CAUSE_RADIO_NETWORK;
      f->cause_value = NGAP_CAUSE_RADIO_NETWORK_XN_HANDOVER_TRIGGERED;
    }
  }

  rrc_gNB_send_XNAP_UE_CONTEXT_RELEASE(instance, ue_context_p);
  return;
}
...
//--------------------------------------------------------------------------------------------
void rrc_gNB_send_XNAP_UE_CONTEXT_RELEASE(instance_t instance, rrc_gNB_ue_context_t *const ue_context_pP)
//----------------------------------------------------------------------------------------
{
  LOG_I(NR_RRC, "Sending UE CONTEXT RELEASE to task XNAP\n");
  gNB_RRC_UE_t *UE = &ue_context_pP->ue_context;
  MessageDef *msg_p = itti_alloc_new_message(TASK_RRC_GNB, 0, XNAP_UE_CONTEXT_RELEASE);

  xnap_ue_context_release_t *ue_release = &XNAP_UE_CONTEXT_RELEASE(msg_p);
  memset(ue_release, 0, sizeof(*ue_release));

  ue_release->s_ng_node_ue_xnap_id = UE->ho_context_xn->source_xn->s_ue_xnap_id;
  ue_release->t_ng_node_ue_xnap_id = UE->ho_context_xn->target_xn->t_ue_xnap_id;
  ue_release->assoc_id = UE->ho_context_xn->source_xn->du->assoc_id;

  xnap_gNB_instance_t *instance_p = xnap_gNB_get_instance(instance);
  msg_p->ittiMsgHeader.originInstance = instance_p->assoc_id_temp;

  if (UE->ho_context != NULL) {
    LOG_I(NR_RRC, "Removing handover context at target..!\n");
    free(UE->ho_context);
    UE->ho_context = NULL;
  }
  itti_send_msg_to_task(TASK_XNAP, instance_p->instance, msg_p);
}
